
<!-- Jquery js-->
<!--<script src="/--><?php //=MOCKUP_PROTECTED_FOLDER?><!--/assets/plugins/jquery/jquery.min.js"></script>-->

<!-- Bootstrap js-->
<script src="/<?=MOCKUP_PROTECTED_FOLDER?>/assets/plugins/bootstrap/js/bootstrap.min.js" async></script>

<!-- Perfect-scrollbar js -->
<script src="/<?=MOCKUP_PROTECTED_FOLDER?>/assets/plugins/perfect-scrollbar/perfect-scrollbar.min.js" async></script>

<!-- Sidemenu js -->
<script src="/<?=MOCKUP_PROTECTED_FOLDER?>/assets/plugins/sidemenu/sidemenu.js" id="leftmenu" async></script>

<!-- Color Theme js -->
<!--<script src="/--><?php //=MOCKUP_PROTECTED_FOLDER?><!--/assets/js/themeColors.js" defer></script>-->

<!-- Custom js -->
<!--<script src="/--><?php //=MOCKUP_PROTECTED_FOLDER?><!--/assets/js/custom.js" defer></script>-->

<?php require_once VIEW_OPEN_BLOCKS."/footer/footer_scripts.php"?>
<!-- Select2 js-->
<script src="/<?=MOCKUP_PROTECTED_FOLDER?>/assets/plugins/select2/js/select2.min.js" async></script>
<script src="/<?=MOCKUP_PROTECTED_FOLDER?>/assets/js/select2.js" async></script>

<!-- Sticky js -->
<script src="/<?=MOCKUP_PROTECTED_FOLDER?>/assets/js/sticky.js" async></script>
